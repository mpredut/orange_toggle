...
<<<<<<< ours
    ...
    recaptcha_info = await page.evaluate("""
        () => {
            ...
            return {
                hasIframe: !!iframe,
                tokenValue: token?.value?.substring(0, 30) || null,
                tokenLength: token?.value?.length || 0
            };
        }
    """)
    log.info(f"  reCAPTCHA: iframe={recaptcha_info['hasIframe']}, token_len={recaptcha_info['tokenLength']}")

    # Force click — ocolește verificarea disabled din Playwright
    # onclick-ul de pe buton va fi executat oricum de browser
...
COOKIES_FILE = "orange_cookies.json"

async def save_cookies(context):
    cookies = await context.cookies()
    with open(COOKIES_FILE, "w") as f:
        json.dump(cookies, f)
    log.info(f"  ✓ Cookies salvate ({len(cookies)} entries)")
...
>>>>>>> theirs
        try:
            ...
            if session_ok:
                # Verifică dacă sesiunea e încă validă
            ...
            if not session_ok:
                await login(page)
                # Verifică că login-ul a reușit înainte să salveze
...
async def init_session():
    """
    Deschide browserul vizibil, așteaptă să te loginezi manual,
    apoi salvează cookies pentru rulările automate ulterioare.
    """
    ...
    async with async_playwright() as p:
        ...
        # Așteaptă input din terminal (non-blocking cu asyncio)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "init":
        # Mod special: deschide browserul, loginează manual, salvează sesiunea
